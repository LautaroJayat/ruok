...
type mapping struct {
	okJobs map[string]int
	l      sync.Mutex
}

func (m *mapping) add(key string) {
	m.l.Lock()
	defer m.l.Unlock()
	v, ok := m.okJobs[key]
	if !ok {
...
func makeTestHandler(results *mapping) gin.HandlerFunc {
	return func(c *gin.Context) {
		results.add(fmt.Sprintf("http://%s%s", c.Request.Host, c.Request.URL.String()))
...
/*
It will mount a testing server that echoes 200 for each job request.
...
func TestCreateMaxJobs_all_succeed(t *testing.T) {
	...
	rows, err := s.GetClient().Query(
		context.Background(),
		"select endpoint, count(id) num from ruok.job_results where succeeded = 'ok' group by endpoint",
	)
	if err != nil {
	...
	for rows.Next() {
		...
		gotten, ok := results.okJobs[endpoint]
		if !ok {
